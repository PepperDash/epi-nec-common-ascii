...
public static class AssemblyFixture
{
    ...
    // Flat <OutputPath> default (bin\$(Configuration)\net8\) -> src/bin/{Config}/net8/...
    private static string PluginDllPath =>
        Path.GetFullPath(Path.Combine(
            AppContext.BaseDirectory,
            "..", "..", "..", "..",
            "src", "bin", Configuration, "net8",
            "epi-nec-common-ascii.4Series.dll"));
    ...
    private static MetadataLoadContext CreateContext()
    {
        ...
        // NOT deps.json - plugin csprojs put <ExcludeAssets>runtime</ExcludeAssets> on the
        // PepperDashEssentials reference (Essentials provides those DLLs on the processor at
        // runtime), so the plugin's own deps.json omits dependencies entirely. project.assets.json
